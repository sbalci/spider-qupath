// spider_classify_annotations_detailed.groovy
// Complete workflow: Export annotations, run SPIDER model, and apply classifications

import qupath.lib.objects.PathObject
import qupath.lib.roi.RectangleROI
import qupath.lib.io.GsonTools
import qupath.fx.dialogs.Dialogs  // Updated to new dialogs package
...
def scriptPath = new File(new File(projectPath).getParent(), "python/spider_qupath_classifier_detailed.py").getAbsolutePath()
...
for (int i = 0; i < jsonArray.size(); i++) {
    ...
    if (!predictionObj.get("prediction").isJsonNull()) {
        ...
        if (predictionObj.has("top_predictions") && !predictionObj.get("top_predictions").isJsonNull()) {
            def topArray = predictionObj.get("top_predictions").getAsJsonArray()
            def topList = []
            
            for (int j = 0; j < topArray.size(); j++) {
                def topObj = topArray.get(j).getAsJsonObject()
                topList.add([
                    'class': topObj.get("class").getAsString(),
                    'probability': topObj.get("probability").getAsDouble()
                ])
            }
            prediction.top_predictions = topList
        }
    } else {
...
def findOrCreatePathClass = { String name ->
    // Use updated PathClass method instead of deprecated PathClassFactory
    ...
    if (pathClass == null) {
        ...
        // Use PathClass.getInstance instead of deprecated PathClassFactory
        pathClass = PathClass.getInstance(name, color)
    }
    return pathClass
}
...
predictions.each { prediction ->
    ...
    if (annotation != null && prediction.prediction != null) {
        ...
        def topPredictionsText = ""
        if (prediction.containsKey('top_predictions') && prediction.top_predictions != null) {
            def topList = prediction.top_predictions
            def filtered = topList.findAll { it.class != prediction.prediction } // Filter out the main prediction
            
            if (filtered.size() > 0) {
                topPredictionsText = filtered.collect { 
                    "${it.class}: ${String.format("%.1f", it.probability * 100)}%" 
                }.join(", ")
            }
        }
        
        // Add confidence information as a comment
        def mainProb = prediction.probabilities[prediction.prediction]
        def mainProbText = String.format("%.1f", mainProb * 100)
        def comment = "${prediction.prediction} (${mainProbText}%)"
        
        // Add other predictions if available
        if (topPredictionsText.length() > 0) {
            comment += " | Others: " + topPredictionsText
        }
        ...
        if (topPredictionsText.length() > 0) {
            println("  Other predictions: " + topPredictionsText)
        }
    } else {
        if (annotation == null) {
